...
import { Loader2, Plus, Lock, Unlock, RefreshCw, AlertTriangle, Play, Calendar, Trash2, Pencil } from 'lucide-react';
...
export function SiklusPeriodeTab({ isCentral }: { isCentral: boolean }) {
    ...
    const handleSavePeriode = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (editingPeriodeId) {
                await supabase.from('periode_anggaran').update(payload).eq('id', editingPeriodeId);
            } else {
                await supabase.from('periode_anggaran').insert({ ...payload, status: 'DRAFT' });
            }
            ...
            alert('Gagal menyimpan periode. Format mungkin sudah ada atau terjadi kesalahan.');
        } finally {
    ...
    const handleOpenEdit = (item: any) => {
        setEditingPeriodeId(item.id);
        setTahunAjaran(item.tahun_ajaran);
        setTanggalMulai(item.tanggal_mulai ? new Date(item.tanggal_mulai).toISOString().split('T')[0] : '');
        setTanggalSelesai(item.tanggal_selesai ? new Date(item.tanggal_selesai).toISOString().split('T')[0] : '');
        setIsModalOpen(true);
    };
                            ...
                            <tr key={item.id} className="hover:bg-slate-50/50">
                                ...
                                <td className="px-4 py-3 text-center">
                                    <div className="flex items-center justify-center gap-2">
                                        ...
                                        <button onClick={() => handleOpenEdit(item)} disabled={isProcessing} className="bg-amber-50 text-amber-600 border border-amber-200 px-2 py-1 rounded-md text-[8px] font-black flex items-center gap-1 hover:bg-amber-100 uppercase disabled:opacity-50">
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50">
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-sm p-4">
                        <h3 className="text-xs font-black text-slate-800 uppercase mb-4">{editingPeriodeId ? 'Edit Tahun Ajaran' : 'Buat Tahun Ajaran Baru'}</h3>
                        <form onSubmit={handleSavePeriode} className="space-y-3">
                            ...
                            <div className="flex gap-2 pt-2">
                                ...
                                <button type="submit" disabled={isProcessing} className="flex-1 py-2 text-xs font-black text-white bg-blue-600 hover:bg-blue-700 rounded-lg disabled:opacity-50">{editingPeriodeId ? 'Simpan' : 'Buat Draft'}</button>
                            </div>
                        </form>
                    </div>
                </div>
