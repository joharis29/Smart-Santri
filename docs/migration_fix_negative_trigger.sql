...
-- Fix trigger sinkronisasi pengeluaran agar mengizinkan angka minus
...
BEGIN
  IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
      ...
      SELECT id INTO v_jenjang_id FROM jenjang WHERE nama_jenjang = NEW.unit LIMIT 1;
      IF v_jenjang_id IS NOT NULL THEN v_unit_id = v_jenjang_id;
      ELSE SELECT id INTO v_unit_id FROM unit WHERE nama_unit = NEW.unit LIMIT 1; END IF;
      ...
      IF TG_OP = 'UPDATE' THEN
          ...
          SELECT id INTO v_jenjang_id FROM jenjang WHERE nama_jenjang = OLD.unit LIMIT 1;
          IF v_jenjang_id IS NOT NULL THEN v_old_unit_id = v_jenjang_id;
          ELSE SELECT id INTO v_old_unit_id FROM unit WHERE nama_unit = OLD.unit LIMIT 1; END IF;

          IF v_old_unit_id IS NULL THEN
              UPDATE dompet_dana SET saldo = saldo + OLD.nominal, updated_at = NOW() WHERE unit_id IS NULL AND kategori = v_old_kategori;
          ELSE
              UPDATE dompet_dana SET saldo = saldo + OLD.nominal, updated_at = NOW() WHERE unit_id = v_old_unit_id AND kategori = v_old_kategori;
          END IF;
      END IF;

      IF v_unit_id IS NULL THEN
          UPDATE dompet_dana SET saldo = saldo - NEW.nominal, updated_at = NOW() WHERE unit_id IS NULL AND kategori = v_kategori;
      ELSE
          UPDATE dompet_dana SET saldo = saldo - NEW.nominal, updated_at = NOW() WHERE unit_id = v_unit_id AND kategori = v_kategori;
      END IF;
  END IF;
...
-- Fix trigger sinkronisasi pendapatan agar mengizinkan angka minus jika ada rollback
